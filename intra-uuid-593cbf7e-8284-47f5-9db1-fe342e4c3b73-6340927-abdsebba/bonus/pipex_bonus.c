...
/*   Updated: 2025/02/15 21:16:26 by abdsebba         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int manage_command_maltu(char **av, char **env, int ac)
{
	...
	pp = malloc(sizeof(pipex));
	if (!open_filee(pp, ac, av) || pp == NULL)
		return 0;
	while (pp->i < (ac - 1))
	{
		if (pp->i < (ac - 2))
		{
			if (pipe(pp->fd) == -1)
				return (write(2, "Pipe() failed!\n", 18), close(pp->infile), close(pp->outfile), -1);
		}
		pp->pid = fork();
		if (pp->pid == -1)
	...
	return (ft_wait(pp->infile, pp->outfile, pp->prev_fd), 1);
}

int	main(int ac, char *av[], char **env)
{
	if (ac < 5 || !valid_arg_bonus(av))
